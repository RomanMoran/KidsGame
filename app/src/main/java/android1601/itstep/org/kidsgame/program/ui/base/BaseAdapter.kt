...
abstract class BaseAdapter<T, VH : BaseViewHolder<T>> : RecyclerView.Adapter<VH> {
    ...
    open fun removeItemInList(item : T){
        items?.remove(item)
    }
    ...
    fun updateItemOnly(position: Int, newItem: T): Boolean {
        if (position < 0 || position >= itemCount) return false
        items?.set(position, newItem)
        return true
    }
    ...
    fun checkCorrectPosition(position: Int): Boolean = (0 <= position && position < items?.size ?: 0)
